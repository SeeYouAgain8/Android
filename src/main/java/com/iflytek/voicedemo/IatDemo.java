...
//	};
	...
	private RecognizerListener mRecognizerListener = new RecognizerListener() {
		...
		public void onResult(RecognizerResult results, boolean isLast) {
			//Log.e("SS------------------------------------------------", results.getResultString());
	...
	private void printResult(RecognizerResult results) { //作为接口，返回字符串
		...
		Log.e("MIatResult-------",mIatResults.toString());
		StringBuffer resultBuffer = new StringBuffer();
		for (String key : mIatResults.keySet()) {
			resultBuffer.append(mIatResults.get(key));
		}

		mResultText.setText(resultBuffer.toString());////返回听写结果， 为String类型！！！！
		mResultText.setSelection(mResultText.length());
...
//		mIat.setParameter(SpeechConstant.ASR_SOURCE_PATH, "sdcard/storage/emulated/0/tts.wav");
		ret = mIat.startListening(mRecognizerListener);
		if (ret != ErrorCode.SUCCESS) {
			showTip("识别失败,错误码：" + ret+",请点击网址https://www.xfyun.cn/document/error-code查询解决方案");
		} else {
			byte[] audioData = FucUtil.readAudioFile(IatDemo.this, "tts.wav");
